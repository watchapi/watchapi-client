 ...
 */
export async function hasPayloadCMS(rootDir: string): Promise<boolean> {
  try {
    const hasPayload = await hasWorkspaceDependency(
      rootDir,
      [...PAYLOAD_DEPENDENCIES],
    );
    if (hasPayload) {
...
function parsePayloadConfig(
  ...
  for (const collection of collections) {
    const collectionRoutes = generateCollectionRoutes(collection, apiPrefix, filePath, rootDir, debug);
    logger.info(`Payload CMS: Collection '${collection.slug}' generated ${collectionRoutes.length} routes`);
  ...
  for (const global of globals) {
    handlers.push(
      ...generateGlobalRoutes(global, apiPrefix, filePath, rootDir, debug),
    );
  }
  ...
  for (const endpoint of endpoints) {
    const handler = generateEndpointRoute(endpoint, apiPrefix, filePath, debug);
    if (handler) {
...
function findPayloadConfigObject(
  ...
  for (const call of callExpressions) {
    ...
    const name = Node.isIdentifier(expression) ? expression.getText() : null;
...
function extractCollections(
  ...
  for (const element of initializer.getElements()) {
    ...
    if (Node.isIdentifier(element) || Node.isPropertyAccessExpression(element)) {
    ...
    if (Node.isSpreadElement(element)) {
      ...
      const spreadCollections = resolveSpreadCollections(spreadExpr, debug);
  ...
  for (const declaration of declarations) {
    ...
    if (Node.isVariableDeclaration(declaration)) {
      ...
      if (initializer && Node.isCallExpression(initializer)) {
        ...
        if (args.length > 0 && Node.isObjectLiteralExpression(args[0])) {
          const sourceFile = declaration.getSourceFile().getFilePath();
          debug(`Resolved ${nodeName} to function call result in ${sourceFile}`);
      ...
      if (initializer && Node.isAsExpression(initializer)) {
        ...
        if (Node.isObjectLiteralExpression(expr)) {
          const sourceFile = declaration.getSourceFile().getFilePath();
          debug(`Resolved ${nodeName} via as expression in ${sourceFile}`);
      ...
      if (initializer && Node.isSatisfiesExpression(initializer)) {
        ...
        if (Node.isObjectLiteralExpression(expr)) {
          const sourceFile = declaration.getSourceFile().getFilePath();
          debug(`Resolved ${nodeName} via satisfies expression in ${sourceFile}`);
          return parseCollectionObject(expr, debug, sourceFile);
        }
      }
      ...
      if (initializer) {
        ...
        if (objLiteral) {
          const sourceFile = declaration.getSourceFile().getFilePath();
          debug(`Resolved ${nodeName} via fallback extraction in ${sourceFile}`);
    ...
    if (Node.isExportAssignment(declaration)) {
      ...
      if (Node.isObjectLiteralExpression(expression)) {
        const sourceFile = declaration.getSourceFile().getFilePath();
        debug(`Resolved ${nodeName} from export default in ${sourceFile}`);
    ...
    if (Node.isImportSpecifier(declaration)) {
      ...
      // Get the source file of the import
      const sourceFile = declaration.getSourceFile();
      const resolvedModule = sourceFile
        .getProject()
        .getSourceFile((sf) => {
          const sfPath = sf.getFilePath();
          return (
            sfPath.includes(moduleSpecifier) ||
            sfPath.endsWith(`${moduleSpecifier}.ts`) ||
            sfPath.endsWith(`${moduleSpecifier}.js`) ||
            sfPath.endsWith(`${moduleSpecifier}/index.ts`) ||
            sfPath.endsWith(`${moduleSpecifier}/index.js`)
          );
        });

      if (resolvedModule) {
        // Look for the exported symbol in the resolved module
        const exportedSymbol = resolvedModule.getExportedDeclarations().get(
          declaration.getName(),
        );
        if (exportedSymbol && exportedSymbol.length > 0) {
          ...
          if (Node.isVariableDeclaration(exportedDecl)) {
            ...
            const objLiteral = initializer
              ...
              : null;
            if (objLiteral) {
              debug(`Resolved imported ${nodeName} from ${resolvedModule.getFilePath()}`);
...
function extractCollectionEndpoints(
  ...
  for (const element of initializer.getElements()) {
    ...
    if (path && method) {
      ...
      debug(`Found collection endpoint: ${method.toUpperCase()} ${path}`);
    }
  }
  ...
  for (const declaration of declarations) {
    if (Node.isVariableDeclaration(declaration)) {
      ...
      if (initializer && Node.isCallExpression(initializer)) {
        ...
        if (args.length > 0 && Node.isObjectLiteralExpression(args[0])) {
          const sourceFile = declaration.getSourceFile().getFilePath();
          debug(`Resolved global ${nodeName} to function call result in ${sourceFile}`);
        ...
        if (exportedSymbol && exportedSymbol.length > 0) {
          ...
          if (Node.isVariableDeclaration(exportedDecl)) {
            ...
            if (init && Node.isObjectLiteralExpression(init)) {
              debug(`Resolved imported global ${nodeName} from ${resolvedModule.getFilePath()}`);
              return parseGlobalObject(init, debug, resolvedModule.getFilePath());
            }
          }
        }
      }
...
function generateCollectionRoutes(
  ...
  for (const op of COLLECTION_OPERATIONS) {
    ...
    handlers.push({
      ...
      headers: op.hasBody ? { "Content-Type": "application/json" } : undefined,
    });
  ...
  for (const endpoint of collection.endpoints) {
    ...
    if (method) {
      ...
      debug(`Generated custom collection endpoint: ${method} ${routePath}`);
    }
  }
...
function generateEndpointRoute(
  ...
  const routePath = endpoint.root ? endpoint.path : `${apiPrefix}${endpoint.path}`;
