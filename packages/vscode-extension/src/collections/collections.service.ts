  ...
  async getAll(): Promise<Collection[]> {
    try {
      ...
      if (isCloud) {
        ...
        logger.info(`Fetched ${collections.length} collections from cloud`);
        return collections;
      } else {
  ...
  async update(id: string, input: UpdateCollectionInput): Promise<Collection> {
    try {
      ...
      if (isCloud) {
        ...
        const collection = await trpc.updateCollection({ id, ...input });
        logger.info(
  ...
  async search(query: string): Promise<Collection[]> {
    try {
      ...
      if (isCloud) {
        ...
        const allCollections = await this.localStorage!.getCollections();
