(function () {
  function onLoad() {
    ...
    const children = Array.from(code.childNodes).filter(
      (n) => n.nodeType === Node.ELEMENT_NODE,
    );

    children
      .filter((n) => n.hasAttribute("range-start"))
      .forEach((n) => n.addEventListener("click", toggleLines));
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    // @ts-ignore
    const lineSpan = childs.find(
      (n) => n.nodeType === 1 && n.hasAttribute("range-start"),
    );
